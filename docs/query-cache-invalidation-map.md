...
| `useCreateSelfCareGoalMutation` | `self_care_goals` | Exact `SelfCareGoals(userId, localDate)` updated with the returned canonical row, unless the new to-do is not due on that date — a weekdays to-do written on a weekend is saved but does not join today's list. A new to-do has no earlier completions, so nothing needs refetching. |
| `useCreateSelfCareGoalsMutation` | `self_care_goals` | Exact `SelfCareGoals(userId, localDate)` appended with the returned canonical rows that are due on that date, then re-sorted. Onboarding's starter plan, written as one insert; like the single write, new rows have no earlier completions, so nothing needs refetching. |
