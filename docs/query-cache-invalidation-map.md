...
| `getSelfCareGoalsQueryKey` | `src/queries/selfCare/useSelfCareGoalsQuery.ts` | Active `self_care_goals` due on the date, plus `self_care_goal_completions` for one user and local date | Personal daily checklist. It does not feed canonical exercise dailies, streaks, History, or feature usage, but it **does** gate the room reward: `useDayCompletion` counts the to-dos alongside the plan's exercises, and a day with an open to-do earns no decoration. Recurrence is applied on read: weekdays to-dos are absent on a weekend, and a one-off is absent once it was completed on an earlier day. |
...
| `useToggleSelfCareGoalMutation` | `self_care_goal_completions` for `localDate` | Exact `SelfCareGoals(userId, localDate)` optimistically updated, and that is the whole refresh: the write is one boolean and the optimistic list is already the shape a refetch returns. Invalidated only after a failure, once the rollback is in. |
