...
If a feature has more than one entry point (Home's Today’s Dailies and Explore's library), **every** entry point gates independently with the same pattern. There is no single chokepoint to lean on.

For Daily Exercise today:
- `HomeScreen.tsx` (Today’s Dailies actions)
- `RoutineLibraryScreen.tsx` (mood-grid action)
...
| Entry-point gates (HR + Daily) | `src/app/navigation/MainTabs.tsx`, `src/screens/HomeScreen.tsx`, `src/screens/RoutineLibraryScreen.tsx`, `src/components/explore/BreathingLibrary.tsx` |
