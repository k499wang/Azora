...
static const size_t kTargetFrameSize = 300;
...
- (NSDictionary*)sampleROI:(HeartRateROI)roi
  ...
  size_t startX = originX + (size_t)floor((double)width * roi.x);
  size_t startY = originY + (size_t)floor((double)height * roi.y);
  ...
  size_t endX = MIN(frameWidth, startX + roiWidth);
  size_t endY = MIN(frameHeight, startY + roiHeight);
...
- (id _Nullable)callback:(Frame*)frame withArguments:(NSDictionary* _Nullable)arguments {
  ...
  size_t sampleWidth = MIN(width, kTargetFrameSize);
  size_t sampleHeight = MIN(height, kTargetFrameSize);
  size_t sampleOriginX = width > sampleWidth ? (width - sampleWidth) / 2 : 0;
  size_t sampleOriginY = height > sampleHeight ? (height - sampleHeight) / 2 : 0;
  HeartRateROI rois[] = {
