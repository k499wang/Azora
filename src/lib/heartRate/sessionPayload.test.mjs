...
test('buildHeartRateSessionRpcPayload builds session, 1s BPM buckets, and timezone-local date', () => {
  ...
  assert.deepEqual(payload.ibi_samples, [
...
test('buildHeartRateSessionRpcPayload falls back to the final reading when no BPM buckets can be derived', () => {
  ...
  assert.ok(payload, 'expected a payload');
  assert.equal(payload.session.avg_bpm, 62);
  assert.equal(payload.session.min_bpm, 62);
  assert.equal(payload.session.max_bpm, 62);
  assert.deepEqual(payload.samples, []);
});
