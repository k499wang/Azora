...
import { ExerciseHeartRateGuidance } from './ExerciseHeartRateGuidance';
import type { BreathingTechnique } from '../../data/techniques';
import type { BreathingPhase } from '../../hooks/useBreathingPhaseRunner';
import type {
...
export const EXERCISE_SESSION_INTRO_DURATION_MS = 750;
...
type ExerciseSessionCamera = Pick<
  HeartRateCameraPreviewProps,
  'device' | 'format' | 'frameProcessor' | 'torchMode'
>;

interface ExerciseSessionHeartRatePresentation {
  enabled: boolean;
  active: boolean;
  bpm: number | null;
  beatTick: number;
  ...
  camera?: ExerciseSessionCamera;
}

interface ExerciseSessionPresentationProps {
  phase: ExerciseSessionPhase;
  technique: BreathingTechnique;
  theme: ExerciseDarkTheme;
  heartRate: ExerciseSessionHeartRatePresentation;
}

const PHASE_LABELS: Record<ExerciseSessionPhase, string> = {
  idle: '',
  intro: '',
  placement: '',
  inhale: 'Inhale',
...
export const ExerciseSessionPresentation = forwardRef<
  BreathingCircleRef,
  ExerciseSessionPresentationProps
>(function ExerciseSessionPresentation(
  ...
  useEffect(() => {
    Animated.timing(transition, {
      toValue: isIdle ? 0 : 1,
      duration: EXERCISE_SESSION_INTRO_DURATION_MS,
