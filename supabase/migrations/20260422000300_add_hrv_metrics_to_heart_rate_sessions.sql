-- Persist derived HRV summary metrics on standalone heart-rate sessions and
-- expose today's latest standalone session + IBI graph for authenticated users.
...
comment on column public.heart_rate_sessions.rmssd is
  'Derived RMSSD in ms for the standalone heart-rate session.';
comment on column public.heart_rate_sessions.sdnn is
  'Derived SDNN in ms for the standalone heart-rate session.';
comment on column public.heart_rate_sessions.pnn50 is
  'Derived pNN50 percentage for the standalone heart-rate session.';
comment on column public.heart_rate_sessions.hr_drop is
  'Derived heart-rate drop (start HR minus end HR) in bpm for the standalone heart-rate session.';
comment on column public.heart_rate_sessions.beat_count is
  'Number of inter-beat intervals used to derive HRV metrics for the standalone heart-rate session.';
...
create or replace view public.user_today_heart_rate_ibi_samples_v as
with profile_days as (
  select
    p.user_id,
    ((now() at time zone p.timezone)::date) as today_local
...
join latest_session ls
  on ls.id = s.heart_rate_session_id
order by s.offset_ms asc;

grant select on public.user_today_heart_rate_v to authenticated;
grant select on public.user_today_heart_rate_ibi_samples_v to authenticated;
